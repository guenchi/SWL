# Fixed bugs w/ invoke of menubutton items, but still some remaining problems.
#   bugs reproducible w/ following script:
...
#     #      1. click on Edit
#     #      2. use arrow keys to move to BUG
#     #      3. hit spacebar / enter
#     #      4. click button w/o moving mouse
#          
...
#
# RCS: @(#) $Id: menu.tcl 1.1 Tue, 11 Jun 2002 20:42:11 -0500 owaddell $
#
...
#-----------
# code lifted from tkMenuMotion to fix bug described at top of file
# running this code ensures that we actually post the menu we're over
    global tkPriv
    set menu %W
    set x %x
    set y %y
...
proc tkMenuMotion {menu x y state} {
	...
	if {[$menu cget -type] == "menubar"} {
	    if {$state & 0x1f00 || \
                ($tkPriv(OW) == 0 && [info exists tkPriv(focus)] && \
		    ([string compare $menu $tkPriv(focus)] != 0))} {
		$menu activate @$x,$y
		tkGenerateMenuSelect $menu
	    }
...
proc tkMenuLeave {menu rootx rooty state} {
    ...
    if {([$menu cget -type] == "menubar" && [$menu type active] == "cascade")} {
       set m [$menu entrycget active -menu]
       if {($m != "" && [winfo ismapped $m])} {
          return
       }
    }
    if {([$menu type active] == "cascade")
...
proc tkMenuInvoke {w buttonRelease} {
	...
	tkMenuUnpost $w
	tkTearOffMenu $w
    } elseif {[$w cget -type] == "menubar" && [$w index active] != "none"} {
	$w postcascade none
        if {[catch {$w entrycget active -command}] == 0} {
	   uplevel #0 [list $w invoke active]
           # if invoked via keyboard, we're done
           if {$buttonRelease == 0} {
	      $w activate none
	      event generate $w <<MenuSelect>>
   	      tkMenuUnpost $w
           } elseif {[$w type active] != "cascade"} {
	      event generate $w <<MenuSelect>>
   	      tkMenuUnpost $w
           }
        } else {
...
proc tkMenuEscape menu {
    ...
    if {([winfo class $parent] != "Menu")} {
	tkMenuUnpost $menu
	# Deactivate if user hits Escape with non-cascade
        # menu item active during keyboard traversal of menubar.
        # If non-cascade item was active due to keyboard traversal
        # and the mouse also happens to be over this item, we should
        # not deactivate, but right now I don't care enough to fix it.
        if {[$menu cget -type] == "menubar"} {
           $menu activate none
        }
...
proc tk_menuSetFocus {menu} {
    ...
    focus $menu
# Maybe this isn't such a great idea... (disables File menu?)
#    if {[$menu cget -type] == "menubar"} {
