;; Copyright (c) 1996 Carl Bruggeman & John Zuckerman
;;
...
;;
;; johnz: corrected handling of "file:" url
;;

;;
;; Exported: read-html-token
;;
...
(define-state (skip-whitespace)
  (with-peek-char c
    (state-case c
      [eof (state-return data (void))]
      [(#\space #\tab #\newline #\return #\page)
       (with-read-char c
	 (*state skip-whitespace))]
      [else
       (state-return data (void))])))
  
(define-state (rd-whitespace i c)
  (state-case c
    [eof 
     (with-stretch-buffer i #\space
       (state-return data (substring tb 0 (fx1+ i))))]
    [(#\space #\tab #\newline #\return #\page)
     (with-read-char c
       (*state rd-whitespace i c))]
    [else
     (with-stretch-buffer i #\space
       (*state rd-data (fx1+ i) c))]))
...
(define-state (rd-name-rest k-succ i j c)
  (with-stretch-buffer j c
    (with-read-char c
      (state-case c
        [eof (*state rd-data (fx1+ j) c)]
        [((#\a - #\z) (#\A - #\Z) (#\0 - #\9) #\. #\- #\_)
         (*state rd-name-rest k-succ i (fx1+ j) c)]
        [else
         (with-unread-char c
           (*state k-succ (fx1+ j) (extr-name tb i (fx1+ j))))]))))
...
(define-state (rd-start-tag-rest i tag attr-list)
  (with-read-char c
    (state-case c
      ...
      [(#\space #\tab #\newline #\return #\page)
       (with-stretch-buffer i c
         (*state rd-start-tag-rest (fx1+ i) tag attr-list))]
  ...
  (define-state (rd-equal1 i name)
    (with-read-char c
      (state-case c
        ...
        [(#\space #\tab #\newline #\return #\page)
         (with-stretch-buffer i c
           (*state rd-equal1 (fx1+ i) name))]
        [(#\=)
         (with-stretch-buffer i c
           (*state rd-equal2 (fx1+ i) name))]
        [(#\>)
         (with-unread-char c
  ...
  (define-state (rd-value i c name)
    (with-stretch-buffer i c
      (state-case c
        ...
        [(#\") (*state rd-value-dquote i (fx1+ i) c name)]
        [(#\') (*state rd-value-squote i (fx1+ i) c name)]
        [else  (*state rd-value-str i (fx1+ i) c name)])))
  (define-state (rd-value-dquote i j c name)
    (with-read-char c
      (state-case c
        [eof (*state rd-data j c)]
        [(#\>) (*state rd-value-end (fx1+ i) j c name)]
        [(#\") (*state rd-value-end+ (fx1+ i) j c name)]
        [else  (with-stretch-buffer j c
                 (*state rd-value-dquote i (fx1+ j) c name))])))
  (define-state (rd-value-squote i j c name)
    (with-read-char c
      (state-case c
        ...
        [(#\') (*state rd-value-end+ (fx1+ i) j c name)]
        [else  (with-stretch-buffer j c
                 (*state rd-value-squote i (fx1+ j) c name))])))
    ...
    (with-read-char c
      (state-case c
        ...
        [(#\space #\tab #\newline #\return #\page)
         (*state rd-value-end i j c name)]
        [else  (with-stretch-buffer j c
                 (*state rd-value-str i (fx1+ j) c name))])))
...
(define-state (rd-token-decl i c)
  (state-case c
    ...
    [(#\-) (with-stretch-buffer i c
             (with-read-char c
               (*state rd-comment-1 (fx1+ i) c)))]
    [else  (with-stretch-buffer i c
             (with-read-char c
               (*state rd-token-decl (fx1+ i) c)))]))
...
(define-state (rd-token-proc-instr i c)
  (state-case c
    ...
    [else  (with-stretch-buffer i c
             (with-read-char c
               (*state rd-token-proc-instr (fx1+ i) c)))]))

(define-state (rd-entity i c)
  (with-stretch-buffer i c
    (with-read-char c
      (state-case c
        [eof   (*state rd-data (fx1+ i) c)]
        [(#\;) (*state rd-data (fx1+ i) c)]
        [((#\a - #\z) (#\A - #\Z)) (*state rd-entity-name (fx1+ i) c)]
        [(#\#) (with-stretch-buffer (fx1+ i) c
                 (with-read-char c
                   (*state rd-entity-num (fx+ i 2) c)))]
        [else (*state rd-data (fx1+ i) c)]))))
        
(define-state (rd-entity-name i c)
  (with-stretch-buffer i c
    (with-read-char c
      (state-case c
        [eof (state-return entity-name (substring tb 1 (fx1+ i)))]
        [(#\;) (state-return entity-name (substring tb 1 (fx1+ i)))]
        [((#\a - #\z) (#\A - #\Z) (#\0 - #\9))
         (*state rd-entity-name (fx1+ i) c)]
  ...
  (with-stretch-buffer i c
    (with-read-char c
      (state-case c
        ...
        [((#\0 - #\9)) (*state rd-entity-num (fx1+ i) c)]
...
(define extr-num
  (lambda (str i j)
    (string->number (substring str i (fx1+ j)))))
  ...
  (with-read-char c
    (state-case c
      ...
      [((#\a - #\z) (#\A - #\Z))
       (with-peek-char c1
         (state-case c1
           [eof (*state rd-uri-type 0 c)]
           [(#\:) ; assume dos drive spec, e.g., c:
            (with-unread-char c
              (*state rd-uri-rest 'file))]
           [else (*state rd-uri-type 0 c)]))]
      [else
...
(define-state (rd-uri-rest type)
  (case type
    ...
    [(file)   (*state rd-url-file-addr rd-file-path type 0)]
...
(define-state (rd-url-addr k-succ type i)
  (with-read-char c
    (state-case c
      ...
      [(#\/) (with-stretch-buffer i c
               (*state rd-url-addr1 k-succ type (fx1+ i)))]
...
(define-state (rd-url-addr1 k-succ type i)
  (with-read-char c
    (state-case c
      [eof (*state rd-info 'url-error i)]
      [(#\/) (with-stretch-buffer i c
               (*state rd-url-addr2 k-succ type (fx1+ i)))]
      [else (with-stretch-buffer i c
              (*state rd-info 'url-error (fx1+ i)))])))
...
(define-state (rd-url-addr2 k-succ type i)
  (with-read-char c
    (state-case c
      ...
      [((#\a - #\z) (#\A - #\Z) (#\0 - #\9))
       (with-stretch-buffer i c
         (*state rd-url-addr3 k-succ type (fx1+ i)))]
      [else (with-stretch-buffer i c
              (*state rd-info 'url-error (fx1+ i)))])))
  ...
  (with-read-char c
    (state-case c
      ...
      [else (with-stretch-buffer i c
              (*state rd-url-addr3 k-succ type (fx1+ i)))]))) 
;;;
;;; File url addrs differ from other addrs
;;;
(define-state (rd-url-file-addr k-succ type i)
  (with-read-char c
    (state-case c
      [eof (*state rd-info 'url-error i)]
      [(#\/) (with-stretch-buffer i c
               (*state rd-url-file-addr1 k-succ type (fx1+ i)))]
      [else (*state k-succ type "" #f 0 0 c)])))

(define-state (rd-url-file-addr1 k-succ type i) ;;; seen 1 slash
  (with-read-char c
    (state-case c
      [eof (*state rd-info 'url-error i)]
      [(#\/) (with-stretch-buffer i c
               (*state rd-url-file-addr2 k-succ type (fx1+ i)))]
      [else (*state k-succ type "" #f 0 1 c)])))

(define-state (rd-url-file-addr2 k-succ type i) ;;; seen 2 slashes
  (with-read-char c
    (state-case c
      [eof (*state k-succ type (substring tb 2 i) #f 0 0 c)]
      [(#\/) (*state k-succ type (substring tb 2 i) #f 0 0 c)]
...
(define-state (rd-url-port k-succ type addr i)
  (with-read-char c
    (state-case c
      ...
      [((#\0 - #\9))
       (with-stretch-buffer i c
         (*state rd-url-port k-succ type addr (fx1+ i)))]
      [else
...
(define-state (rd-http-path type addr port i j c)
  (state-case c
    ...
    [else (with-stretch-buffer j c
            (with-read-char c
              (*state rd-http-path type addr port i (fx1+ j) c)))]))

(define-state (rd-file-path type addr port i j c)
  (state-case c
    [eof (*state rd-location type addr port (substring tb i j) 0)]
    [else (with-stretch-buffer j c
            (with-read-char c
              (*state rd-file-path type addr port i (fx1+ j) c)))]))
...
(define-state (rd-location type addr port path i)
  (with-read-char c
    (state-case c
      ...
      [else (with-stretch-buffer i c
              (*state rd-location type addr port path (fx1+ i)))])))
...
(define-state (rd-query type addr port path i)
  (with-read-char c
    (state-case c
      ...
      [else (with-stretch-buffer i c
              (*state rd-query type addr port path (fx1+ i)))])))
  ...
  (case-lambda
    ...
    [(ip pre?)
     (unless (input-port? ip)
       (assertion-violationf 'read-html-token "~s is not an input port" ip))
     (let ((tb "") (fp (most-negative-fixnum)) (pre? pre?))
       (*state/k rd-html-token parse-html))]))

(set! skip-html-whitespace
  (case-lambda
    [(ip)
     (unless (input-port? ip)
       (assertion-violationf 'skip-html-whitespace "~s is not an input port" ip))
     (let ((tb "") (fp (most-negative-fixnum)) (pre? #f))
       (*state/k skip-whitespace parse-html))]))
...
(define merge-url
  (lambda (base-url url)
    (case (url-type url)
      [rel
       (unless (url? base-url) 
	 (assertion-violationf 'merge-url "can't use a relative address without a base url"))
       (let ([base-type (url-type base-url)]
	     [base-addr (url-addr base-url)]
	     [base-port (url-port base-url)]
	     [loc (url-loc url)]
	     [query (url-query url)])
	 (let ([fn (path-merge (url-path base-url) (url-path url))])
	   (make-url base-type base-addr base-port fn loc query)))]
      [http
       (when (eqv? (url-path url) "") (set-url-path! url "/"))
       url]
      [else url])))
...
(define url->string
  (lambda (url)
    (let ((typ (symbol->string (url-type url))))
    (string-append
      (case (url-type url)
	[(mailto news) (string-append typ ":")]
	[(rel) ""]
	[else (string-append typ "://")])
      (url-addr url)
